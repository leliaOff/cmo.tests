<template>
    <div class="results">
        ...
        <div class="row">
            <div class="col-sm-6 clearfix"><label for="setting-nr">Скрыть нулевые результаты?</label></div>
            <div class="col-sm-6 clearfix"><input type="checkbox" id="setting-nr" v-model="setting.nr" @change="settingChange"/></div>
        </div>

        <h2>Общая статистика</h2>
        <div class="row">
            <div class="col-sm-6 clearfix"><label>Общее число респондентов, прошедших тест</label></div>
            <div class="col-sm-2 clearfix"><label>{{ general.countPeople }}</label></div>
            ...
            <div class="col-sm-2 clearfix"><a class="btn btn-text" title="Выгрузка" :href="'/getExcel/' + testId">Выгрузить</a></div>
        </div>
        <div class="row">
            <div class="col-sm-6 clearfix"><label>Последнее обновление: </label></div>
            <div class="col-sm-6 clearfix"><label>{{ general.tm }}</label></div>
        </div>        
        <div class="row">
            <div class="col-sm-6 clearfix"><label>Число вопросов</label></div>
            <div class="col-sm-6 clearfix"><label>{{ general.countElements }}</label></div>
        </div>
        ...
        <div class="row" v-for="result in linksResults" :key="result.id" v-if="result.count > 0">
            <div class="col-sm-6 clearfix"><label>{{ result.title }}</label></div>
            <div class="col-sm-2 clearfix"><label>{{ result.count }}</label></div>
            ...
            <div class="col-sm-2 clearfix"><a class="btn btn-text" title="Выгрузка" :href="'/getExcel/' + testId + '/' + result.alias + '/' + result.id">Выгрузить</a></div>
        </div>
        
        <h2>Подробная статистика</h2>
        <div class="element-item" v-for="(item, i) in items" :key="'result-' + item.id" v-if="item.type != 'title'">
